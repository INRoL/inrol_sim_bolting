#include "inrol_sim_bolting/inrol_sim_bolting.hpp"
#include <ros/ros.h>
#include <fstream>
#include <iostream>
#include <string>
...
#include "inrol_sim_bolting_main/msg_for_joint.h"
...
inrol_sim_bolting_main::msg_for_joint msg_joint_angle;
void publish();
...
int main(int argc, char** argv) {
	...
	joint_angle_pub = n.advertise<inrol_sim_bolting_main::msg_for_joint>("/franka_joint_angle",100);
  ...
  file3.open((asset_root + "/des/Rd_gripper.txt").c_str());
  ...
  while (true)
  {
    for (int i = 0; i < 3; i++)
    {
      ...
      {
        file2 >> Rd[i][j];
        file3 >> Rd_gripper[i][j];
      }
    }

    inrol_sim::sim_function(pd, Rd, Rd_gripper, xk_sim, qk_sim, ak_sim, Fc_sim);
    publish();
		loop_rate.sleep();
